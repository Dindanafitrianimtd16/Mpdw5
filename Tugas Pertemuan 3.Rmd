...
Dari hasil tersebut, didapat bahwa peubah $x_t$ dan $y_{t-1}$ memiliki nilai $P-Value<$. Hal ini menunjukkan bahwa peubah $x_t$ dan $y_{t-1}$ berpengaruh signifikan terhadap $y$. Adapun model keseluruhannya adalah sebagai berikut

$$
\hat{Y_t}= 0.2049+0.7074 X_t+0.26971Y_{t-1}
$$
...
```{r}
fore.koyck <- forecast(model = model.koyck, x=test$Xt, h=40) #
fore.koyck
...
```{r}
model.dlm <- dlm(x = train$Xt,y = train$Yt , q = 2)  #Q=JUMLAH X
summary(model.dlm)
AIC(model.dlm)
BIC(model.dlm)
```
...
$$
\hat{Y_t}=-0.0008948+1.1257271X_t-0.1244068X_{t-1}-0.0135481X_{t-2}
$$
...
fore.dlm
mape.dlm <- MAPE(fore.dlm$forecasts, test$Yt) #YT YG DI DATA test
#akurasi data training
GoF(model.dlm)
```
...
$$
\hat{Y_t}=0.001043+1.137203X_t+...-0.016356X_{t-6}
$$
...
fore.dlm2
mape.dlm2<- MAPE(fore.dlm2$forecasts, test$Yt) #mape punya testing
mape.dlm
#akurasi data training
GoF(model.dlm2)
```
...
Hasil di atas menunjukkan bahwa selain peubah $x_{t}$, hasil uji t menunjukkan nilai-p pada peubah $\ge0.05$ Hal ini menunjukkan bahwa peubah $x_{t-1}$ berpengaruh signifikan terhadap $y_t$, sementara $x_t$ dan $y_{t-1}$ tidak berpengaruh signifikan terhadap $y_t$. Model keseluruhannya adalah sebagai berikut:

$$
\hat{Y}=-0.002359 + 1.125440X_t-0.153197X_{t-1}+0.015933Y_{t-1}
$$
...
Dari tabel di atas, dapat terlihat bahwa nilai AIC terendah didapat ketika $p=6$ dan $q=1$, yaitu sebesar `-324.8979`. Artinya, model autoregressive optimum didapat ketika $p=6$ dan $q=1$.
...
Berdasarkan nilai MAPE, model paling optimum didapat pada Model DLM1 karena memiliki nilai MAPE yang terkecil.
...
Berdasarkan plot tersebut, terlihat bahwa plot yang paling mendekati data aktualnya adalah DLM 1, sehingga dapat disimpulkan model terbaik dalam hal ini adalah model DLM1
